...
Manager::~Manager()
{
    if (memoryUsage() == Mocha::MemoryUsage::MultipleLayouts) {
        m_importer->setMultipleLayoutsStatus(Mocha::MultipleLayouts::Paused);
    }
...
{
    Mocha::Data::GenericTable<Data::Generic> viewtemplates = m_corona->templatesManager()->viewTemplates();
...
Mocha::Data::LayoutIcon Manager::iconForLayout(const QString &storedLayoutName) const
{
    Data::Layout l = m_synchronizer->data(storedLayoutName);
    return iconForLayout(l);
}

Mocha::Data::LayoutIcon Manager::iconForLayout(const Data::Layout &layout) const
{
    Mocha::Data::LayoutIcon _icon;
...
void Manager::moveView(QString originLayoutName, uint originViewId, QString destinationLayoutName)
{
    ...
    Mocha::View *originview = originlayout->viewForContainment(originViewId);
...
{
    if (m_mochaSettingsWindow) {
        m_mochaSettingsWindow->deleteLater();
        m_mochaSettingsWindow = nullptr;
    }
}
