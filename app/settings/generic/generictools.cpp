...
void drawFormattedText(QPainter *painter, const QStyleOption &option, const QString &text, Qt::AlignmentFlag alignment, const float textOpacity)
{
    ...
    QPalette::ColorRole applyColor = Mocha::isSelected(option) ? QPalette::HighlightedText : QPalette::Text;
    QBrush nBrush = option.palette.brush(Mocha::colorGroup(option), applyColor);
...
{
    bool active = Mocha::isActive(option);
    bool selected = Mocha::isSelected(option);
    bool focused = Mocha::isFocused(option);
    ...
    if (isBackgroundFile) {
        ...
        pen.setColor(option.palette.color(Mocha::colorGroup(option), textColorRole));
...
QRect drawScreen(QPainter *painter, const QStyleOption &option, bool drawMultipleScreens, QRect screenGeometry, const int &maxIconSize, const float brushOpacity)
{
    ...
    QColor pencolor = option.palette.color(Mocha::colorGroup(option), textColorRole);
