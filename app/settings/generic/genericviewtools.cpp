...
void drawView(QPainter *painter, const QStyleOption &option, const Mocha::Data::View &view, const QRect &availableScreenRect, const float brushOpacity)
{
    int thick = 4;
    painter->save();
    ...
    QColor pencolor = option.palette.color(Mocha::colorGroup(option), viewColorRole);
    ...
    if (view.alignment != Mocha::Types::Justify) {
        length = 0.5 * length;
    }
    ...
    if (view.isHorizontal()) {
        ...
        } else if (view.alignment == Mocha::Types::Justify) {
            x = availableScreenRect.x() + thick / 2;
        }
    } else if (view.isVertical()) {
        if (view.alignment == Mocha::Types::Top) {
