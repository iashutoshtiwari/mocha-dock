...
const Mocha::Data::View Views::appendViewFromViewTemplate(const Data::View &view)
{
...
Data::ViewsTable Views::selectedViewsForClipboard()
{
    ...
    Mocha::Data::Layout currentlayout = m_handler->currentData();
    ...
    for(int i=0; i<selectedviews.rowCount(); ++i) {
        ...
        Mocha::Data::View copiedview = selectedviews[i];
...
void Views::duplicateSelectedViews()
{
    ...
    for(int i=0; i<selectedviews.rowCount(); ++i) {
        if (selectedviews[i].state() == Data::View::IsCreated) {
            QString storedviewpath = m_handler->layoutsController()->templatesKeeper()->storedView(currentlayout.id, selectedviews[i].id);
            Mocha::Data::View duplicatedview = selectedviews[i];
...
{   
    ...
    Mocha::CentralLayout *currentlayout = m_handler->layoutsController()->centralLayout(currentlayoutdata.id);
...
int Views::viewsForRemovalCount() const
{
    ...
    Mocha::Data::ViewsTable originalViews = m_model->originalViewsData();
    Mocha::Data::ViewsTable currentViews = m_model->currentViewsData();
...
CentralLayout *Views::originLayout(const Data::View &view)
{
    ...
    Mocha::CentralLayout *originactive = m_handler->layoutsController()->isLayoutOriginal(origincurrentid) ?
...
void Views::messagesForErrorsWarnings(const Mocha::CentralLayout *centralLayout, const bool &showNoErrorsMessage)
{
    if (!centralLayout) {
        return;
    }
...
void Views::save()
{
    ...
    Mocha::Data::Layout originallayout = m_handler->originalData();
    Mocha::Data::Layout currentlayout = m_handler->currentData();
    Mocha::CentralLayout *central = m_handler->layoutsController()->centralLayout(currentlayout.id);

    //! views in model
    Mocha::Data::ViewsTable originalViews = m_model->originalViewsData();
    Mocha::Data::ViewsTable currentViews = m_model->currentViewsData();
    Mocha::Data::ViewsTable alteredViews = m_model->alteredViews();
    Mocha::Data::ViewsTable newViews = m_model->newViews();
    ...
    for(const auto vid: cuttedpastedviews.keys()){
        ...
        Mocha::CentralLayout *origin = m_handler->layoutsController()->centralLayout(originlayout.id);

        Data::ViewsTable originviews = Mocha::Layouts::Storage::self()->views(origin);
