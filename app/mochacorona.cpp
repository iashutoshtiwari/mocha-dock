      ...
      m_dialogShadows(new PanelShadows(this, QStringLiteral("dialogs/background")))
{
    ...
    KPackage::Package package(new Mocha::Package(this));
    ...
    new MochaDockAdaptor(this);
    QDBusConnection dbus = QDBusConnection::sessionBus();
    dbus.registerObject(QStringLiteral("/Mocha"), this);
}
...
void Corona::onAboutToQuit()
{
    ...
    if (m_layoutsManager->memoryUsage() == Mocha::MemoryUsage::MultipleLayouts) {
                                                  ...
                                                  bool desktopUse) const
{
    ...
    if (!ignoreModes.contains(Mocha::Types::None)) {
        ignoreModes << Mocha::Types::None;
    }

    if (!ignoreModes.contains(Mocha::Types::NormalWindow)) {
        ignoreModes << Mocha::Types::NormalWindow;
    }
        ...
        if (view && view->containment() && view->screen() == screen && !inDesktopOffScreenStartup
            ...
            case Plasma::Types::Horizontal:
                if (view->behaveAsPlasmaPanel()) {
                    ...
                    case Mocha::Types::Left:
                        x = view->x() + offsetW;
                        break;

                    case Mocha::Types::Center:
                    case Mocha::Types::Justify:
                        x = (view->geometry().center().x() - w/2) + 1 + offsetW;
                        break;

                    case Mocha::Types::Right:
...
void Corona::onAvailableScreenRegionChangedFrom(Mocha::View *view)
{
...
void Corona::onAvailableScreenRectChangedFrom(Mocha::View *view)
{
    Plasma::Corona* corona = qobject_cast<Plasma::Corona*>(this);
    int screenId = view->positioner()->currentScreenId();
...
QStringList Corona::contextMenuData(const uint &containmentId)
{
    ...
    for(const auto &layoutName : m_layoutsManager->synchronizer()->menuLayouts()) {
        if (m_layoutsManager->synchronizer()->centralLayout(layoutName)
                || m_layoutsManager->memoryUsage() == Mocha::MemoryUsage::SingleLayout) {
...
QStringList Corona::viewTemplatesData()
{
    ...
    Mocha::Data::GenericTable<Data::Generic> viewtemplates = m_templatesManager->viewTemplates();
...
void Corona::moveViewToLayout(const uint &containmentId, const QString &layoutName)
{
    ...
    if (view && !layoutName.isEmpty() && view->layout()->name() != layoutName) {
        ...
        view->positioner()->setNextLocation(layoutName, screensgroup, "", Plasma::Types::Floating, Mocha::Types::NoneAlignment);
    }
}
...
{   
    ...
    qmlRegisterAnonymousType<Mocha::ViewPart::TrackerPart::CurrentScreenTracker>("mocha-dock", 1);
    qmlRegisterAnonymousType<Mocha::ViewPart::TrackerPart::AllScreensTracker>("mocha-dock", 1);
    qmlRegisterAnonymousType<Mocha::WindowSystem::SchemeColors>("mocha-dock", 1);
    qmlRegisterAnonymousType<Mocha::WindowSystem::Tracker::LastActiveWindow>("mocha-dock", 1);
    qmlRegisterAnonymousType<Mocha::Types>("mocha-dock", 1);
