...
void GenericLayout::setCorona(Mocha::Corona *corona)
{
    m_corona = corona;
}
...
Mocha::Corona *GenericLayout::corona() const
{
    return m_corona;
}
...
Mocha::View *GenericLayout::lastConfigViewFor()
{
    return m_lastConfigViewFor;
}

void GenericLayout::setLastConfigViewFor(Mocha::View *view)
{
    ...
    for (int i = 0; i < sortedViews.size(); ++i) {
        for (int j = 0; j < sortedViews.size() - i - 1; ++j) {
            if (viewAtLowerScreenPriority(sortedViews[j], sortedViews[j + 1], primaryScreen)
                ...
                Mocha::View *temp = sortedViews[j + 1];
...
bool GenericLayout::viewAtLowerScreenPriority(Mocha::View *test, Mocha::View *base, QScreen *primaryScreen)
{
    if (!base || ! test) {
        return true;
    }
...
bool GenericLayout::viewAtLowerEdgePriority(Mocha::View *test, Mocha::View *base)
{
    if (!base || ! test) {
        return true;
    }
...
bool GenericLayout::viewDataAtLowerScreenPriority(const Mocha::Data::View &test, const Mocha::Data::View &base) const
{
...
bool GenericLayout::viewDataAtLowerStatePriority(const Mocha::Data::View &test, const Mocha::Data::View &base) const
{
...
bool GenericLayout::viewDataAtLowerEdgePriority(const Mocha::Data::View &test, const Mocha::Data::View &base) const
{
    QList<Plasma::Types::Location> edges{Plasma::Types::RightEdge, Plasma::Types::TopEdge,
                Plasma::Types::LeftEdge, Plasma::Types::BottomEdge};
...
QList<Mocha::Data::View> GenericLayout::sortedViewsData(const QList<Mocha::Data::View> &viewsData)
{
    QList<Mocha::Data::View> sortedData = viewsData;
    ...
    for (int i = 0; i < sortedData.size(); ++i) {
        for (int j = 0; j < sortedData.size() - i - 1; ++j) {
            if (viewDataAtLowerStatePriority(sortedData[j], sortedData[j + 1])
                ...
                Mocha::Data::View temp = sortedData[j + 1];
...
QList<Mocha::View *> GenericLayout::viewsWithPlasmaShortcuts()
{
...
QString GenericLayout::mapScreenName(const ViewsMap *map, uint viewId) const
{
    ...
    return QString::number(Mocha::ScreenPool::NOSCREENID);
}
...
Data::View GenericLayout::newView(const Mocha::Data::View &nextViewData)
{
...
void GenericLayout::updateView(const Mocha::Data::View &viewData)
{
    ...
    Mocha::View *view = viewForContainment(viewData.id.toUInt());
    ...
    if (view) {
        if (!viewMustBeDeleted) {
            QString scrName = Mocha::Data::Screen::ONPRIMARYNAME;
...
Mocha::Data::ViewsTable GenericLayout::viewsTable() const
{
