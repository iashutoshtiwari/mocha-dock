...
import org.kde.mocha.abilities.bridge as AbilityBridge
...
Item{
    ...
    // NAME: version
    //   USAGE: read-only
    //   EXPLANATION: Mocha communication version in order for the applet to use only properties
    ...
    // NAME: inEditMode
    //   USAGE: read-only
    //   EXPLANATION: Mocha sets it to true when this applet is in a Mocha containment and Mocha
    //       is also in EditMode, that means when the user is altering applets and Mocha mochaView settings
    // @since: 0.9
    readonly property bool inEditMode: appletItem.isMochaPlasmoid ? root.editMode : root.inConfigureAppletsMode

    // NAME: inPlasmaPanelStyle
    //   USAGE: read-only
    //   EXPLANATION: Mocha sets it to true when this view behaves as Plasma panel concerning
    ...
    // NAME: palette
    //   USAGE: read-only
    //   EXPLANATION: Mocha updates it to its coloring palette in order for the applet
    //       to take responsibility of its coloring.
    //   USE CASE: when Mocha is transparent and applets colors need to be adjusted in order
    //       to look consistent with the underlying desktop background
    // @since: 0.9
    readonly property QtObject palette: !applet.mochaSideColoringEnabled ? colorizerManager : null

    // NAME: applyPalette
    //   USAGE: read-only
    //   EXPLANATION: Mocha updates it to TRUE when the applet must use the provided
    //       Mocha "palette" and FALSE otherwise
    //   USE CASE: when Mocha is transparent and applets colors need to be adjusted in order
    //       to look consistent with the underlying desktop background
    // @since: 0.9
    readonly property bool applyPalette: !applet.mochaSideColoringEnabled ? colorizerManager.mustBeShown : false

    // NAME: parabolicEffectEnabled
    //   USAGE: read-only
    //   EXPLANATION: Parabolic Effect is enabled or not for this Mocha View
    ...
    // NAME: iconSize
    //   USAGE: read-only
    //   EXPLANATION: The current icon size used in the Mocha View
    ...
    // @since: 0.9
    readonly property QtObject windowsTracker: applet.windowsTrackingEnabled && mochaView && mochaView.windowsTracker ?
                                                   mochaView.windowsTracker : null
    ...
    Component.onCompleted: {
        if (appletContainsMochaBridge) {
            appletRootItem.mochaBridge = settings;
        }
    }

    Component.onDestruction: {
        if (appletContainsMochaBridge) {
            appletRootItem.mochaBridge = null;
        }
    }
}
