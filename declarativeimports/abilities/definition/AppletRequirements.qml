...
Item{
    ...
    property bool mochaSideColoringEnabled: true
    ...
    //   EXPLANATION: when is TRUE, Mocha can show its own Active Indicator
    //       when needed. For FALSE, the Mocha Active Indicator is not drawn
    //       or used for that applet.
    //   SINCE: 0.9

    //   NAME: mochaSideColoringEnabled
    //   EXPLANATION: when is FALSE, Mocha is not painting/colorizing this applet
    //       in any case. In such case the applet can use 'palette'
    //       in order to access the color palette used at all cases from Mocha
    ...
    //       its own and Mocha should not take any special actions to help it
    //       work properly.
    //   SINCE: 0.10

    //   NAME: windowsTrackingEnabled
    //   EXPLANATION: when is TRUE, Mocha is informed that applet requests
